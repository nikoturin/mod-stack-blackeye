...
{
    var isWindowContainedInDivFrame = (window.parentAccesible && parent && parent.NS && parent.NS.Window.getActive());
    
        
        return (isWindowContainedInDivFrame ? parent.NS.Window.getActive().content.clientHeight : window.innerHeight);
...
function getDocumentClientWidth()
{
	var isWindowContainedInDivFrame = (window.parentAccesible && parent && parent.NS && parent.NS.Window.getActive());
        ...
        return (isWindowContainedInDivFrame ? parent.NS.Window.getActive().content.clientWidth : window.innerWidth);
...
function onlyUSPhoneFormat(field, type, doalert, autoplace, validflag) // former code from 'if' extracted to this function
{
	var val = field.value;
	...
	if (autoplace && validflag) {
		...
		if (re.test(val)) {
			...
			if (digits.length == 7)
				field.value = phoneformat.replace(phoneformat.substring(0, phoneformat.indexOf('4')), '')
					.replace('45' + String.fromCharCode(3), digits.substring(0, 3))
					.replace('789' + String.fromCharCode(3), digits.substring(3)) + ext;
			else if (digits.length == 10)
				field.value = phoneformat.replace('12' + String.fromCharCode(3), digits.substring(0, 3))
				...
				field.value = '1 ' + phoneformat.replace('12' + String.fromCharCode(3), digits.substring(1, 4))
					.replace('45' + String.fromCharCode(3), digits.substring(4, 7))
					.replace('789' + String.fromCharCode(3), digits.substring(7)) + ext;
		}
	}
	return validflag;
}
...
function old_validate_field(field, type, doalert, autoplace, minval, maxval, mandatory, separator, scale)
{
    ...
    else if (type == "phone" || type == "fullphone")
    {
	    ...
	    validflag = onlyUSPhoneFormat(field, type, doalert, autoplace, validflag);
...
function getEventKeypress(evnt)
{
	...
	if ( keyCode === 0 && evnt.key.length > 0) {
		keyCode = evnt.key.toUpperCase().charCodeAt(0);
    }
	return keyCode;
...
function setRequired(fld,required)
{
	...
	else if ( window.getHtmlEditor != null && getHtmlEditor( fld.name ) != null && getHtmlEditor(fld.name).setMandatory)
	{
		...
		setFieldLabelRequired(fld.name, required);
	}
...
function disableSelect(sel, val, win)
{
	if (!isBackend && (sel != null))
	{
		...
		var newlink = sel.form.querySelector('#' + sel.name + "_popup_new");
		if (newlink != null)
			newlink.style.visibility = val ? "hidden" : "inherit";
		var linklink = sel.form.querySelector('#' + sel.name + "_popup_link");
		if (linklink != null)
...
function getExtWindowFrameName(name)
{
	return name + '_frame';
}
...
function nlExtOpenNewUIWindow(args)
{
	...
	extListeners.afterrender = function() {
		var frame = document.getElementById(getExtWindowFrameName(args.winname));
		setOldToNewUIHandshake(frame.contentWindow, deferred);
	};
...
function nlExtOpenWindow(url, winname, width, height, fld, scrollbars, winTitle, listeners, triggerObj)
{
	...
	// close, beforeclose, afterrender
	Object.keys(listeners).forEach(function(eventName) {
		var originalHandler = listeners[eventName];
		listeners[eventName] = function(/*extWindow*/) {
			originalHandler(nsWindow);
		};
	});
	...
	var extWindow = new Ext.Window({
		...
		alignTarget: triggerObj,
		defaultAlign: triggerObj? 'tl-tl?' : 'c-c',
		layout: 'fit',
		items: [{
			xtype: 'box',
			id: getExtWindowFrameName(winname),
			autoEl: {
				tag: 'iframe',
				src: url,
			},
			cls: ['uir-ext-window-body', 'uir-ext-frame-window-body']
		}]
	});
	...
	var nsWindow = NS.Window.__ExtWindow2NsWindow(extWindow);
	return nsWindow;
}

NS.Window = (function (nsWindow) {
	var extWinToNsWinMap = {};

	nsWindow.__ExtWindow2NsWindow = function __ExtWindow2NsWindow(extWindow) {
		var id = extWindow && extWindow.getId();
		if (id && !extWinToNsWinMap[id]) {
			extWindow.on({'close': { fn: function() { delete extWinToNsWinMap[id]; }, single: true }});
			var nsWindow = extWinToNsWinMap[id] = {
				on: function (event, fn) {
					extWindow.on(event, function() {
						fn(nsWindow);
					});
				},
				id: id,
				element: extWindow.getEl().dom,
				content: extWindow.items.first().getEl().dom,
				close: function () { extWindow.close(); },
			};
		}
		return id && extWinToNsWinMap[id];
	};

	nsWindow.get = function get(id) {
		return window.Ext && nsWindow.__ExtWindow2NsWindow(Ext.WindowMgr.get(id));
	};
	...
	nsWindow.getActive = function getActive() {
		return window.Ext && nsWindow.__ExtWindow2NsWindow(Ext.WindowMgr.getActive());
	};
...
NS.LoadMask = {
	create: function (options) {
		var loadMask = new Ext.LoadMask({
			msg: options.text ? options.text : 'Loading...',
			target: options.target ? Ext.get(options.target) : Ext.getBody(),
		});
		return {
			show: function () { loadMask.show(); },
			hide: function () { loadMask.hide(); },
		}
	}
};

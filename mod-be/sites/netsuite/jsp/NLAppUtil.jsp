...
var ns_domain = 'https://system.na5.netsuite.com';
...
function nsResizeIframeToContent(iframeId)
{
    ...
    if (height > 0)
    {
        ...
        if (contentTable)
            iframe.width = contentTable.offsetWidth;
        if (respawn)
...
(function ($) {
    ...
    $(document).ready(function () {
	    ...
	    Tooltip.prototype.open = function () {
		    var maxWidth = Math.min($(window).width(), maxTooltipWidth);
			/* replace <br> line breaks with "\n" - <br> <BR> <br /> <BR /> <br/> <BR/>*/
		    var tooltipText = this.target.getAttribute("data-ns-tooltip").replace(/<[bB][rR]\s?\/?>/g, "\n");
		    var $content = $("<div/>").text(tooltipText).css({
			    overflow: "auto",
			    whiteSpace: 'pre-wrap'
		    });
		    var $wrapper = $("<div/>").append($content).css({
			    display: "flex",
		    ...
		    this.$tooltip = $("<div/>").append($chevron).append($wrapper).css({
			    position: "fixed",
			    display: "flex",
			    zIndex: 1001,
                fontSize: '12px'
		    ...
		    this.box = this.$tooltip.get(0).getBoundingClientRect();
		    this.position();
		    this.positionHandle = setInterval(this.update.bind(this), updateDelay);
	    }
	    ...
	    Tooltip.prototype.contains = function (element) {
		    return this.target.contains(element) || this.$tooltip.get(0).contains(element);
	    }

	    Tooltip.prototype.position = function () {
		    var targetBox = this.target.getBoundingClientRect();
		    var tooltipBox = this.box;
		    var height = Math.min(targetBox.top - 10, tooltipBox.height);

		    this.$tooltip.css({
			    top: Math.floor(targetBox.top - height - 10) + 'px',
			    left: Math.floor(targetBox.left + targetBox.width / 2 - tooltipBox.width / 2) + 'px',
			    maxHeight: height + 'px'
		    });
	    }

	    Tooltip.prototype.update = function () {
		    if (document.contains(this.target) && hasTooltip(this.target)) {
			    this.position();
		    } else {
			    hide();
		    }
	    ...
	    function handleMouseMove(target) {
		    ...
		    var tooltipElement = findTooltipElement(target);
		    if (tooltip && tooltip.contains(updateElement) && (!tooltipElement || tooltipElement === tooltip.target)) {
			    cancelUpdate();
		    } else if (tooltipElement) {
			    scheduleUpdate(() => show(tooltipElement));
		    } else {
			    scheduleUpdate(() => hide());
		    }
	    }
	    ...
	    function show(target) {
		    hide();
			if (document.contains(target) && hasTooltip(target)) {
		...
		function hasTooltip(elem) {
			return elem.getAttribute("data-ns-tooltip") && !elem.classList.contains("uir-machine-focused-cell");
        }

	    window.addEventListener('mousemove', e => handleMouseMove(e.target), true);
    });
}(NS.jQuery));
...
function showPerformancePopupWindow()
{
    nlShowPet('', 'get_nlPetContent().submitAsCase()', performanceResultsAsTable);
}
